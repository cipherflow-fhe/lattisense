...
_p3 = CkksParam.create_default_param(n=16384)
_p3.set_slots(2048)
...
_CKKS_PARAM_TAGS = {
    ...
    id(_p3): f'ckks_param_default_n{_p3.n}_slots{_p3.slots}',
}
...
class TestTask:
    ...
    def test_cap(self, param, lv):
        ...
        task_dir = os.path.join(CPU_OUTPUT_BASE_DIR, param_tag, f'CKKS_{N_OP}_cap', f'level_{lv}')
    ...
    @pytest.mark.min_level(0)
    def test_cap_ringt(self, param):
        set_fhe_param(param)
        param_tag = _param_tag(param)
        for lv in range(0, param.max_level + 1):
            task_dir = os.path.join(CPU_OUTPUT_BASE_DIR, param_tag, f'CKKS_{N_OP}_cap_ringt', f'level_{lv}')
            x_list = [CkksCiphertextNode(f'x_{i}', level=lv) for i in range(N_OP)]
    ...
    def test_cac(self, param, lv):
        set_fhe_param(param)
        param_tag = _param_tag(param)
        task_dir = os.path.join(CPU_OUTPUT_BASE_DIR, param_tag, f'CKKS_{N_OP}_cac', f'level_{lv}')
        x_list = [CkksCiphertextNode(f'x_{i}', level=lv) for i in range(N_OP)]
    ...
    def test_casc(self, param, lv):
        set_fhe_param(param)
        param_tag = _param_tag(param)
        task_dir = os.path.join(CPU_OUTPUT_BASE_DIR, param_tag, f'CKKS_{N_OP}_casc', f'level_{lv}')
        x_list = [CkksCiphertextNode(f'x_{i}', level=lv) for i in range(N_OP)]
    ...
    def test_csp(self, param, lv):
        set_fhe_param(param)
        param_tag = _param_tag(param)
        task_dir = os.path.join(CPU_OUTPUT_BASE_DIR, param_tag, f'CKKS_{N_OP}_csp', f'level_{lv}')
        x_list = [CkksCiphertextNode(f'x_{i}', level=lv) for i in range(N_OP)]
        y_list = [CkksPlaintextNode(f'y_{i}', level=lv) for i in range(N_OP)]
    ...
    @pytest.mark.min_level(0)
    def test_csp_ringt(self, param):
        set_fhe_param(param)
        param_tag = _param_tag(param)
        for lv in range(0, param.max_level + 1):
            task_dir = os.path.join(CPU_OUTPUT_BASE_DIR, param_tag, f'CKKS_{N_OP}_csp_ringt', f'level_{lv}')
            x_list = [CkksCiphertextNode(f'x_{i}', level=lv) for i in range(N_OP)]
            y_list = [CkksPlaintextRingtNode(f'y_{i}') for i in range(N_OP)]
            z_list = [sub(x_list[i], y_list[i], f'z_{i}') for i in range(N_OP)]
            process_custom_task(
    ...
    def test_csc(self, param, lv):
        set_fhe_param(param)
        param_tag = _param_tag(param)
        task_dir = os.path.join(CPU_OUTPUT_BASE_DIR, param_tag, f'CKKS_{N_OP}_csc', f'level_{lv}')
        x_list = [CkksCiphertextNode(f'x_{i}', level=lv) for i in range(N_OP)]
        y_list = [CkksCiphertextNode(f'y_{i}', level=lv) for i in range(N_OP)]
        z_list = [sub(x_list[i], y_list[i], f'z_{i}') for i in range(N_OP)]
        process_custom_task(
    ...
    def test_cneg(self, param, lv):
        set_fhe_param(param)
        param_tag = _param_tag(param)
        task_dir = os.path.join(CPU_OUTPUT_BASE_DIR, param_tag, f'CKKS_{N_OP}_cneg', f'level_{lv}')
        x_list = [CkksCiphertextNode(f'x_{i}', level=lv) for i in range(N_OP)]
    ...
    def test_cmp_ringt(self, param, lv):
        set_fhe_param(param)
        param_tag = _param_tag(param)
        task_dir = os.path.join(CPU_OUTPUT_BASE_DIR, param_tag, f'CKKS_{N_OP}_cmp_ringt', f'level_{lv}')
        x_list = [CkksCiphertextNode(f'x_{i}', level=lv) for i in range(N_OP)]
        y_list = [CkksPlaintextRingtNode(f'y_{i}') for i in range(N_OP)]
    ...
    def test_cmp(self, param, lv):
        set_fhe_param(param)
        param_tag = _param_tag(param)
        task_dir = os.path.join(CPU_OUTPUT_BASE_DIR, param_tag, f'CKKS_{N_OP}_cmp', f'level_{lv}')
        x_list = [CkksCiphertextNode(f'x_{i}', level=lv) for i in range(N_OP)]
        y_list = [CkksPlaintextNode(f'y_{i}', level=lv) for i in range(N_OP)]
        z_list = [mult(x_list[i], y_list[i], f'z_{i}') for i in range(N_OP)]
        process_custom_task(
    ...
    def test_cmp_mul(self, param, lv):
        set_fhe_param(param)
        param_tag = _param_tag(param)
        task_dir = os.path.join(CPU_OUTPUT_BASE_DIR, param_tag, f'CKKS_{N_OP}_cmp_mul', f'level_{lv}')
        x_list = [CkksCiphertextNode(f'x_{i}', level=lv) for i in range(N_OP)]
    ...
    @pytest.mark.min_level(1)
    def test_ct_pt_mac(self, param, lv):
        if param is not _p1:
            pytest.skip('only runs for default param (n=16384)')
        set_fhe_param(param)
        param_tag = _param_tag(param)
        for m in range(2, 21):
            task_dir = os.path.join(CPU_OUTPUT_BASE_DIR, param_tag, f'CKKS_cmpac', f'level_{lv}_m_{m}')
            c_list = [CkksCiphertextNode(f'c_{i}', level=lv) for i in range(m)]
            p_list = [CkksPlaintextNode(f'p_{i}', level=lv) for i in range(m)]
    ...
    def test_ct_pt_ringt_mac(self, param, lv):
        ...
        param_tag = _param_tag(param)
        for m in range(2, 21):
            task_dir = os.path.join(CPU_OUTPUT_BASE_DIR, param_tag, f'CKKS_cmpac_ringt', f'level_{lv}_m_{m}')
            c_list = [CkksCiphertextNode(f'c_{i}', level=lv) for i in range(m)]
    ...
    def test_cmc(self, param, lv):
        set_fhe_param(param)
        param_tag = _param_tag(param)
        task_dir = os.path.join(CPU_OUTPUT_BASE_DIR, param_tag, f'CKKS_{N_OP}_cmc', f'level_{lv}')
        x_list = [CkksCiphertextNode(f'x_{i}', level=lv) for i in range(N_OP)]
        y_list = [CkksCiphertextNode(f'y_{i}', level=lv) for i in range(N_OP)]
        z_list = [mult(x_list[i], y_list[i], f'z_{i}') for i in range(N_OP)]
        process_custom_task(
    ...
    def test_cmc_relin(self, param, lv):
        set_fhe_param(param)
        param_tag = _param_tag(param)
        task_dir = os.path.join(CPU_OUTPUT_BASE_DIR, param_tag, f'CKKS_{N_OP}_cmc_relin', f'level_{lv}')
        x_list = [CkksCiphertextNode(f'x_{i}', level=lv) for i in range(N_OP)]
        y_list = [CkksCiphertextNode(f'y_{i}', level=lv) for i in range(N_OP)]
    ...
    def test_cmc_relin_rescale(self, param, lv):
        set_fhe_param(param)
        param_tag = _param_tag(param)
        task_dir = os.path.join(CPU_OUTPUT_BASE_DIR, param_tag, f'CKKS_{N_OP}_cmc_relin_rescale', f'level_{lv}')
        x_list = [CkksCiphertextNode(f'x_{i}', level=lv) for i in range(N_OP)]
        y_list = [CkksCiphertextNode(f'y_{i}', level=lv) for i in range(N_OP)]
    ...
    def test_csqr(self, param, lv):
        set_fhe_param(param)
        param_tag = _param_tag(param)
        task_dir = os.path.join(CPU_OUTPUT_BASE_DIR, param_tag, f'CKKS_{N_OP}_csqr', f'level_{lv}')
        x_list = [CkksCiphertextNode(f'x_{i}', level=lv) for i in range(N_OP)]
    ...
    def test_csqr_relin(self, param, lv):
        set_fhe_param(param)
        param_tag = _param_tag(param)
        task_dir = os.path.join(CPU_OUTPUT_BASE_DIR, param_tag, f'CKKS_{N_OP}_csqr_relin', f'level_{lv}')
        x_list = [CkksCiphertextNode(f'x_{i}', level=lv) for i in range(N_OP)]
    ...
    def test_csqr_relin_rescale(self, param, lv):
        set_fhe_param(param)
        param_tag = _param_tag(param)
        task_dir = os.path.join(CPU_OUTPUT_BASE_DIR, param_tag, f'CKKS_{N_OP}_csqr_relin_rescale', f'level_{lv}')
        x_list = [CkksCiphertextNode(f'x_{i}', level=lv) for i in range(N_OP)]
    ...
    def test_rescale(self, param, lv):
        set_fhe_param(param)
        param_tag = _param_tag(param)
        task_dir = os.path.join(CPU_OUTPUT_BASE_DIR, param_tag, f'CKKS_{N_OP}_rescale', f'level_{lv}')
        x_list = [CkksCiphertextNode(f'x_{i}', level=lv) for i in range(N_OP)]
    ...
    def test_drop_level(self, param, lv, drop_lv=2):
        ...
        task_dir = os.path.join(
            CPU_OUTPUT_BASE_DIR, param_tag, f'CKKS_{N_OP}_drop_level', f'level_{lv}', f'drop_{drop_lv}'
        )
        x_list = [CkksCiphertextNode(f'x_{i}', level=lv) for i in range(N_OP)]
    ...
    def test_rotate_col(self, param, lv, steps=[i + 1 for i in range(8)]):
        ...
        param_tag = _param_tag(param)
        task_dir = os.path.join(
            CPU_OUTPUT_BASE_DIR, param_tag, f'CKKS_{N_OP}_rotate_col', f'level_{lv}', f'steps_{steps[0]}_to_{steps[-1]}'
        )
        x_list = [CkksCiphertextNode(f'x_{i}', level=lv) for i in range(N_OP)]
    ...
    def test_advanced_rotate_col(self, param, lv, steps=[-500, 20, 200, 2000, 4000]):
        ...
        task_dir = os.path.join(
            CPU_OUTPUT_BASE_DIR, param_tag, f'CKKS_{N_OP}_advanced_rotate_col', f'level_{lv}', f'steps_{steps_str}'
        )
        x_list = [CkksCiphertextNode(f'x_{i}', level=lv) for i in range(N_OP)]
    ...
    def test_rotate_row(self, param, lv):
        set_fhe_param(param)
        param_tag = _param_tag(param)
        task_dir = os.path.join(CPU_OUTPUT_BASE_DIR, param_tag, f'CKKS_{N_OP}_rotate_row', f'level_{lv}')
        x_list = [CkksCiphertextNode(f'x_{i}', level=lv) for i in range(N_OP)]
    ...
    @pytest.mark.min_level(1)
    def test_custom_encode_and_cap(self, param, lv):
        if param is not _p1:
            pytest.skip('only runs for default param (n=16384)')
        set_fhe_param(param)
        param_tag = _param_tag(param)
        task_dir = os.path.join(CPU_OUTPUT_BASE_DIR, param_tag, f'CKKS_{N_OP}_custom_encode_and_cap', f'level_{lv}')
        x_list = [CkksCiphertextNode(f'x_{i}', level=lv) for i in range(N_OP)]
        y_list = [CustomDataNode(type='msg', id=f'y_{i}') for i in range(N_OP)]
        z_list = []
        for i in range(N_OP):
            encoded_y = CkksPlaintextNode(f'encoded_y_{i}', level=x_list[i].level)
            custom_compute(
                inputs=[y_list[i]],
                output=encoded_y,
                type='encode',
                attributes={'level': x_list[i].level, 'scale': pow(2.0, 34)},
            )
    ...
    def test_n_poly(self, param, lv):
        ...
        set_fhe_param(param)
        param_tag = _param_tag(param)
        task_dir = os.path.join(CPU_OUTPUT_BASE_DIR, param_tag, 'CKKS_n_poly', f'level_{lv}')
    ...
    def test_cap_2d(self, param, lv, row=1, col=1):
        ...
        set_fhe_param(param)
        param_tag = _param_tag(param)
        task_dir = os.path.join(CPU_OUTPUT_BASE_DIR, param_tag, f'CKKS_cap_row_{row}_col_{col}', f'level_{lv}')
    ...
    def test_cap_3d(self, param, lv, dim0=1, dim1=1, dim2=1):
        ...
        set_fhe_param(param)
        param_tag = _param_tag(param)
        task_dir = os.path.join(CPU_OUTPUT_BASE_DIR, param_tag, 'CKKS_cap_3d', f'level_{lv}')
    ...
    def test_cmc_relin_4d(self, param, lv, dim0=1, dim1=1, dim2=1, dim3=1):
        ...
        set_fhe_param(param)
        param_tag = _param_tag(param)
        task_dir = os.path.join(CPU_OUTPUT_BASE_DIR, param_tag, 'CKKS_cmc_relin_4d', f'level_{lv}')
        x_ct_4d = [
    ...
    def test_cmp_3d(self, param, lv, dim0=1, dim1=1, dim2=1):
        ...
        set_fhe_param(param)
        param_tag = _param_tag(param)
        task_dir = os.path.join(CPU_OUTPUT_BASE_DIR, param_tag, 'CKKS_cmp_3d', f'level_{lv}')
        x_ct_3d = [[[CkksCiphertextNode(level=lv) for _ in range(dim2)] for _ in range(dim1)] for _ in range(dim0)]
        y_pt_3d = [[[CkksPlaintextNode(level=lv) for _ in range(dim2)] for _ in range(dim1)] for _ in range(dim0)]
        z_ct_3d = [
    ...
    def test_cmp_4d(self, param, lv, dim0=1, dim1=1, dim2=1, dim3=1):
        ...
        set_fhe_param(param)
        param_tag = _param_tag(param)
        task_dir = os.path.join(CPU_OUTPUT_BASE_DIR, param_tag, 'CKKS_cmp_4d', f'level_{lv}')
        x_ct_4d = [
            [[[CkksCiphertextNode(level=lv) for _ in range(dim3)] for _ in range(dim2)] for _ in range(dim1)]
            for _ in range(dim0)
        ]
...
class TestBootstrap:
    ...
    def test_toy_bootstrap(self, lv):
        set_fhe_param(_p_toy_btp)
        param_tag = f'ckks_param_btp_n{_p_toy_btp.n}'
        task_dir = os.path.join(CPU_OUTPUT_BASE_DIR, param_tag, f'CKKS_{N_OP}_toy_bootstrap', f'level_{lv}')
        x_list = [CkksCiphertextNode(f'x_{i}', level=lv) for i in range(N_OP)]
    ...
    @pytest.mark.parametrize('lv', [0], ids=['lv0'])
    def test_bootstrap(self, lv):
        set_fhe_param(_p_btp)
        param_tag = f'ckks_param_btp_n{_p_btp.n}'
        task_dir = os.path.join(CPU_OUTPUT_BASE_DIR, param_tag, f'CKKS_{N_OP}_bootstrap', f'level_{lv}')
        x_list = [CkksCiphertextNode(f'x_{i}', level=lv) for i in range(N_OP)]
        y_list = [bootstrap(x_list[i], f'y_{i}') for i in range(N_OP)]
        process_custom_task(
            input_args=[Argument('in_x_list', x_list)],
    ...
    def test_cmc_relin_rescale_bootstrap(self, lv):
        set_fhe_param(_p_btp)
        param_tag = f'ckks_param_btp_n{_p_btp.n}'
        task_dir = os.path.join(
            CPU_OUTPUT_BASE_DIR, param_tag, f'CKKS_{N_OP}_cmc_relin_rescale_bootstrap', f'level_{lv}'
        )
        x_list = [CkksCiphertextNode(f'x_{i}', level=lv) for i in range(N_OP)]
        y_list = [CkksCiphertextNode(f'y_{i}', level=lv) for i in range(N_OP)]
        result_list = []
    ...
    @pytest.mark.parametrize('lv', [0], ids=['lv0'])
    def test_toy_sparse_bootstrap(self, lv):
        set_fhe_param(_p_toy_sparse_btp)
        param_tag = f'ckks_param_btp_n{_p_toy_sparse_btp.n}_slots{_p_toy_sparse_btp.slots}'
        task_dir = os.path.join(CPU_OUTPUT_BASE_DIR, param_tag, f'CKKS_{N_OP}_toy_bootstrap', f'level_{lv}')
        x_list = [CkksCiphertextNode(f'x_{i}', level=lv) for i in range(N_OP)]
        y_list = [bootstrap(x_list[i], f'y_{i}') for i in range(N_OP)]
        process_custom_task(
    ...
    @pytest.mark.parametrize('lv', [0], ids=['lv0'])
    def test_sparse_bootstrap(self, lv):
        set_fhe_param(_p_sparse_btp)
        param_tag = f'ckks_param_btp_n{_p_sparse_btp.n}_slots{_p_sparse_btp.slots}'
        task_dir = os.path.join(CPU_OUTPUT_BASE_DIR, param_tag, f'CKKS_{N_OP}_bootstrap', f'level_{lv}')
        x_list = [CkksCiphertextNode(f'x_{i}', level=lv) for i in range(N_OP)]
        y_list = [bootstrap(x_list[i], f'y_{i}') for i in range(N_OP)]
        process_custom_task(
    ...
    @pytest.mark.parametrize('lv', [3], ids=['lv3'])
    def test_sparse_cmc_relin_rescale_bootstrap(self, lv):
        set_fhe_param(_p_sparse_btp)
        param_tag = f'ckks_param_btp_n{_p_sparse_btp.n}_slots{_p_sparse_btp.slots}'
        task_dir = os.path.join(
            CPU_OUTPUT_BASE_DIR, param_tag, f'CKKS_{N_OP}_cmc_relin_rescale_bootstrap', f'level_{lv}'
