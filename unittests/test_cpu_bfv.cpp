...
TEMPLATE_TEST_CASE_METHOD(BfvFixture, "BFV cac level error", "", BfvTestDefaultParams) {
    auto xv = new_bfv_test_ct(this->n_op, this->ctx, 2, this->param.get_t());
    auto yv = new_bfv_test_ct(this->n_op, this->ctx, 2, this->param.get_t());
    vector<BfvCiphertext> z_list;
    z_list.reserve(this->n_op);
    for (int _i = 0; _i < this->n_op; _i++)
        z_list.push_back(this->ctx.new_ciphertext(2));
    string project_path = cpu_base_path + "/" + this->tag + "/BFV_" + to_string(this->n_op) + "_cac/level_3";
    ...
    REQUIRE_THROWS_WITH(cpu_project.run(&this->ctx, cxx_args),
...
TEMPLATE_TEST_CASE_METHOD(BfvFixture, "BFV cap", "", BfvTestDefaultParams, BfvTestCustomParams) {
    for (int level = this->min_level; level <= this->max_level; level++) {
        SECTION("lv=" + to_string(level)) {
            ...
            string path =
                cpu_base_path + "/" + this->tag + "/BFV_" + to_string(this->n_op) + "_cap/level_" + to_string(level);
...
TEMPLATE_TEST_CASE_METHOD(BfvFixture, "BFV cap_ringt", "", BfvTestDefaultParams, BfvTestCustomParams) {
    for (int level = this->min_level; level <= this->max_level; level++) {
        SECTION("lv=" + to_string(level)) {
            ...
            for (int _i = 0; _i < this->n_op; _i++)
                z_list.push_back(this->ctx.new_ciphertext(level));
            string path = cpu_base_path + "/" + this->tag + "/BFV_" + to_string(this->n_op) + "_cap_ringt/level_" +
                          to_string(level);
            FheTaskCpu proj(path);
            vector<CxxVectorArgument> args = {
                {"in_x_list", &xv.ciphertexts},
    ...
    for (int level = this->min_level; level <= this->max_level; level++) {
        SECTION("lv=" + to_string(level)) {
            ...
            for (int _i = 0; _i < this->n_op; _i++)
                z_list.push_back(this->ctx.new_ciphertext(level));
            string path =
                cpu_base_path + "/" + this->tag + "/BFV_" + to_string(this->n_op) + "_cac/level_" + to_string(level);
            FheTaskCpu proj(path);
            vector<CxxVectorArgument> args = {
                {"in_x_list", &xv.ciphertexts},
    ...
    for (int level = this->min_level; level <= this->max_level; level++) {
        SECTION("lv=" + to_string(level)) {
            ...
            for (int _i = 0; _i < this->n_op; _i++)
                z_list.push_back(this->ctx.new_ciphertext(level));
            string path =
                cpu_base_path + "/" + this->tag + "/BFV_" + to_string(this->n_op) + "_casc/level_" + to_string(level);
            FheTaskCpu proj(path);
            vector<CxxVectorArgument> args = {
                {"in_x_list", &xv.ciphertexts},
                {"out_z_list", &z_list},
    ...
    for (int level = this->min_level; level <= this->max_level; level++) {
        SECTION("lv=" + to_string(level)) {
            ...
            for (int _i = 0; _i < this->n_op; _i++)
                z_list.push_back(this->ctx.new_ciphertext(level));
            string path =
                cpu_base_path + "/" + this->tag + "/BFV_" + to_string(this->n_op) + "_csp/level_" + to_string(level);
            FheTaskCpu proj(path);
            vector<CxxVectorArgument> args = {
                {"in_x_list", &xv.ciphertexts},
                {"in_y_list", &yv.plaintexts},
...
TEMPLATE_TEST_CASE_METHOD(BfvFixture, "BFV csp_ringt", "", BfvTestDefaultParams, BfvTestCustomParams) {
    for (int level = this->min_level; level <= this->max_level; level++) {
        SECTION("lv=" + to_string(level)) {
            ...
            for (int _i = 0; _i < this->n_op; _i++)
                z_list.push_back(this->ctx.new_ciphertext(level));
            string path = cpu_base_path + "/" + this->tag + "/BFV_" + to_string(this->n_op) + "_csp_ringt/level_" +
                          to_string(level);
            FheTaskCpu proj(path);
            vector<CxxVectorArgument> args = {
                {"in_x_list", &xv.ciphertexts},
    ...
    for (int level = this->min_level; level <= this->max_level; level++) {
        SECTION("lv=" + to_string(level)) {
            ...
            for (int _i = 0; _i < this->n_op; _i++)
                z_list.push_back(this->ctx.new_ciphertext(level));
            string path =
                cpu_base_path + "/" + this->tag + "/BFV_" + to_string(this->n_op) + "_csc/level_" + to_string(level);
            FheTaskCpu proj(path);
            vector<CxxVectorArgument> args = {
                {"in_x_list", &xv.ciphertexts},
                {"in_y_list", &yv.ciphertexts},
    ...
    for (int level = this->min_level; level <= this->max_level; level++) {
        SECTION("lv=" + to_string(level)) {
            ...
            for (int _i = 0; _i < this->n_op; _i++)
                z_list.push_back(this->ctx.new_ciphertext(level));
            string path =
                cpu_base_path + "/" + this->tag + "/BFV_" + to_string(this->n_op) + "_cssc/level_" + to_string(level);
            FheTaskCpu proj(path);
            vector<CxxVectorArgument> args = {
                {"in_x_list", &xv.ciphertexts},
                {"out_z_list", &z_list},
    ...
    for (int level = this->min_level; level <= this->max_level; level++) {
        SECTION("lv=" + to_string(level)) {
            ...
            for (int _i = 0; _i < this->n_op; _i++)
                z_list.push_back(this->ctx.new_ciphertext(level));
            string path =
                cpu_base_path + "/" + this->tag + "/BFV_" + to_string(this->n_op) + "_cneg/level_" + to_string(level);
            FheTaskCpu proj(path);
            vector<CxxVectorArgument> args = {
                {"in_x_list", &xv.ciphertexts},
                {"out_z_list", &z_list},
...
TEMPLATE_TEST_CASE_METHOD(BfvFixture, "BFV cmp_ringt", "", BfvTestDefaultParams, BfvTestCustomParams) {
    for (int level = 1; level <= this->max_level; level++) {
        SECTION("lv=" + to_string(level)) {
            ...
            for (int _i = 0; _i < this->n_op; _i++)
                z_list.push_back(this->ctx.new_ciphertext(level));
            string path = cpu_base_path + "/" + this->tag + "/BFV_" + to_string(this->n_op) + "_cmp_ringt/level_" +
                          to_string(level);
            FheTaskCpu proj(path);
            vector<CxxVectorArgument> args = {
                {"in_x_list", &xv.ciphertexts},
    ...
    for (int level = 1; level <= this->max_level; level++) {
        SECTION("lv=" + to_string(level)) {
            ...
            for (int _i = 0; _i < this->n_op; _i++)
                z_list.push_back(this->ctx.new_ciphertext(level));
            string path =
                cpu_base_path + "/" + this->tag + "/BFV_" + to_string(this->n_op) + "_cmp/level_" + to_string(level);
            FheTaskCpu proj(path);
            vector<CxxVectorArgument> args = {
                {"in_x_list", &xv.ciphertexts},
                {"in_y_list", &yv.plaintexts},
...
TEMPLATE_TEST_CASE_METHOD(BfvFixture, "BFV cmp_mul", "", BfvTestDefaultParams, BfvTestCustomParams) {
    for (int level = 1; level <= this->max_level; level++) {
        SECTION("lv=" + to_string(level)) {
            auto xv = new_bfv_test_ct(this->n_op, this->ctx, level, this->param.get_t());
            ...
            for (int _i = 0; _i < this->n_op; _i++)
                z_list.push_back(this->ctx.new_ciphertext(level));
            string path = cpu_base_path + "/" + this->tag + "/BFV_" + to_string(this->n_op) + "_cmp_mul/level_" +
                          to_string(level);
            FheTaskCpu proj(path);
            vector<CxxVectorArgument> args = {
                {"in_x_list", &xv.ciphertexts},
...
TEMPLATE_TEST_CASE_METHOD(BfvFixture, "BFV cmc", "", BfvTestDefaultParams, BfvTestCustomParams) {
    for (int level = 1; level <= this->max_level; level++) {
        SECTION("lv=" + to_string(level)) {
            ...
            string path =
                cpu_base_path + "/" + this->tag + "/BFV_" + to_string(this->n_op) + "_cmc/level_" + to_string(level);
            FheTaskCpu proj(path);
            vector<CxxVectorArgument> args = {
                {"in_x_list", &xv.ciphertexts},
                {"in_y_list", &yv.ciphertexts},
...
TEMPLATE_TEST_CASE_METHOD(BfvFixture, "BFV cmc_relin", "", BfvTestDefaultParams, BfvTestCustomParams) {
    for (int level = 1; level <= this->max_level; level++) {
        SECTION("lv=" + to_string(level)) {
            ...
            for (int _i = 0; _i < this->n_op; _i++)
                z_list.push_back(this->ctx.new_ciphertext(level));
            string path = cpu_base_path + "/" + this->tag + "/BFV_" + to_string(this->n_op) + "_cmc_relin/level_" +
                          to_string(level);
            FheTaskCpu proj(path);
            vector<CxxVectorArgument> args = {
                {"in_x_list", &xv.ciphertexts},
...
TEMPLATE_TEST_CASE_METHOD(BfvFixture, "BFV cmc_relin_rescale", "", BfvTestDefaultParams, BfvTestCustomParams) {
    for (int level = 1; level <= this->max_level; level++) {
        SECTION("lv=" + to_string(level)) {
            ...
            string path = cpu_base_path + "/" + this->tag + "/BFV_" + to_string(this->n_op) +
    ...
    for (int level = 1; level <= this->max_level; level++) {
        SECTION("lv=" + to_string(level)) {
            ...
            for (int _i = 0; _i < this->n_op; _i++)
                z_list.push_back(this->ctx.new_ciphertext3(level));
            string path =
                cpu_base_path + "/" + this->tag + "/BFV_" + to_string(this->n_op) + "_csqr/level_" + to_string(level);
            FheTaskCpu proj(path);
            vector<CxxVectorArgument> args = {
                {"in_x_list", &xv.ciphertexts},
                {"out_z_list", &z_list},
...
TEMPLATE_TEST_CASE_METHOD(BfvFixture, "BFV csqr_relin", "", BfvTestDefaultParams, BfvTestCustomParams) {
    for (int level = 1; level <= this->max_level; level++) {
        SECTION("lv=" + to_string(level)) {
            ...
            for (int _i = 0; _i < this->n_op; _i++)
                z_list.push_back(this->ctx.new_ciphertext(level));
            string path = cpu_base_path + "/" + this->tag + "/BFV_" + to_string(this->n_op) + "_csqr_relin/level_" +
                          to_string(level);
            FheTaskCpu proj(path);
            vector<CxxVectorArgument> args = {
                {"in_x_list", &xv.ciphertexts},
...
TEMPLATE_TEST_CASE_METHOD(BfvFixture, "BFV rescale", "", BfvTestDefaultParams, BfvTestCustomParams) {
    for (int level = 1; level <= this->max_level; level++) {
        SECTION("lv=" + to_string(level)) {
            ...
            string path = cpu_base_path + "/" + this->tag + "/BFV_" + to_string(this->n_op) + "_rescale/level_" +
                          to_string(level);
            FheTaskCpu proj(path);
            vector<CxxVectorArgument> args = {
                {"in_x_list", &xv.ciphertexts},
...
TEMPLATE_TEST_CASE_METHOD(BfvFixture, "BFV rotate_col", "", BfvTestDefaultParams, BfvTestCustomParams) {
    ...
    for (int level = 1; level <= this->max_level; level++) {
        SECTION("lv=" + to_string(level)) {
            ...
            string path = cpu_base_path + "/" + this->tag + "/BFV_" + to_string(this->n_op) + "_rotate_col/level_" +
...
TEMPLATE_TEST_CASE_METHOD(BfvFixture, "BFV rotate_row", "", BfvTestDefaultParams, BfvTestCustomParams) {
    ...
    for (int level = 1; level <= this->max_level; level++) {
        SECTION("lv=" + to_string(level)) {
            ...
            string path = cpu_base_path + "/" + this->tag + "/BFV_" + to_string(this->n_op) + "_rotate_row/level_" +
                          to_string(level);
            FheTaskCpu proj(path);
            vector<CxxVectorArgument> args = {
                {"arg_x", &xv.ciphertexts},
...
TEMPLATE_TEST_CASE_METHOD(BfvFixture, "BFV ctc_ctc_0", "", BfvTestDefaultParams) {
    ...
    SECTION("lv=3") {
        ...
        FheTaskCpu proj(cpu_base_path + "/" + this->tag + "/BFV_ctc_ctc_0/level_3");
        vector<CxxVectorArgument> args = {
            {"in_x_list", &xv.ciphertexts},
            {"in_y_list", &yv.ciphertexts},
            {"out_z_list", &z_list},
...
TEMPLATE_TEST_CASE_METHOD(BfvFixture, "BFV ctc_ctc_1", "", BfvTestDefaultParams) {
    ...
    SECTION("lv=3") {
        ...
        FheTaskCpu proj(cpu_base_path + "/" + this->tag + "/BFV_ctc_ctc_1/level_3");
        vector<CxxVectorArgument> args = {
            {"in_x_list", &xv.ciphertexts},
            {"in_y_list", &yv.ciphertexts},
            {"out_z_list", &z_list},
...
TEMPLATE_TEST_CASE_METHOD(BfvFixture, "BFV 1_square_square", "", BfvTestDefaultParams) {
    ...
    SECTION("lv=3") {
        ...
        FheTaskCpu proj(cpu_base_path + "/" + this->tag + "/BFV_1_square_square/level_3");
        vector<CxxVectorArgument> args = {
            {"in_x_list", &xv.ciphertexts},
            {"out_z_list", &z_list},
        };
...
TEMPLATE_TEST_CASE_METHOD(BfvFixture, "BFV 1_ctc_rotate_cac", "", BfvTestDefaultParams) {
    ...
    SECTION("lv=3") {
        ...
        FheTaskCpu proj(cpu_base_path + "/" + this->tag + "/BFV_1_ctc_rotate_cac/level_3");
        vector<CxxVectorArgument> args = {
            {"in_x_list", &xv.ciphertexts},
            {"in_y_list", &yv.ciphertexts},
            {"out_z_list", &z_list},
...
TEMPLATE_TEST_CASE_METHOD(BfvFixture, "BFV double", "", BfvTestDefaultParams) {
    SECTION("lv=1") {
        ...
        FheTaskCpu proj(cpu_base_path + "/" + this->tag + "/BFV_1_double");
        vector<CxxVectorArgument> args = {
            {"in_x_list", &xv.ciphertexts},
            {"out_z_list", &z_list},
        };
...
TEMPLATE_TEST_CASE_METHOD(BfvFixture, "BFV braid", "", BfvTestDefaultParams) {
    ...
    SECTION("lv=3") {
        ...
        FheTaskCpu proj(cpu_base_path + "/" + this->tag + "/BFV_braid");
        vector<CxxVectorArgument> args = {
...
TEMPLATE_TEST_CASE_METHOD(BfvFixture, "BFV poly", "", BfvTestDefaultParams) {
    ...
    SECTION("lv=3") {
        ...
        FheTaskCpu proj(cpu_base_path + "/" + this->tag + "/BFV_n_poly/level_3");
        vector<CxxVectorArgument> args = {
            {"in_x_list", &xv.ciphertexts},
...
TEMPLATE_TEST_CASE_METHOD(BfvFixture, "BFV poly_2", "", BfvTestDefaultParams) {
    if (this->max_level < 5)
        return;

    SECTION("lv=5") {
        auto xv = new_bfv_test_ct(1, this->ctx, 5, this->param.get_t());
        auto av = new_bfv_test_pt_mul(2, this->ctx, 5, this->param.get_t());
        vector<BfvCiphertext> z_list;
        z_list.reserve(1);
        for (int _i = 0; _i < 1; _i++)
            z_list.push_back(this->ctx.new_ciphertext(5));

        FheTaskCpu proj(cpu_base_path + "/" + this->tag + "/BFV_poly_2/level_5");
        vector<CxxVectorArgument> args = {
            {"in_x", &xv.ciphertexts},
            {"in_coeffs", &av.plaintexts},
            {"out_y", &z_list},
        };
        proj.run(&this->ctx, args);
        ...
        vector<vector<uint64_t>> expected(1);
        auto x2 = vec_mod_mul(xv.values[0], xv.values[0], this->param.get_t());
        auto ax = vec_mod_mul(av.values[0], xv.values[0], this->param.get_t());
        auto ax2 = vec_mod_mul(av.values[1], x2, this->param.get_t());
        expected[0] = vec_mod_add(ax, ax2, this->param.get_t());
        REQUIRE(decrypt_and_decode(this->ctx, z_list) == expected);
    }
}
...
TEMPLATE_TEST_CASE_METHOD(BfvFixture, "BFV ct_pt_ringt_mac", "", BfvTestDefaultParams) {
    for (int m = 44; m <= 50; m++) {
        SECTION("m=" + to_string(m) + "/lv=1") {
            ...
            string path = cpu_base_path + "/" + this->tag + "/BFV_cmpac/level_1_m_" + to_string(m);
            FheTaskCpu proj(path);
            vector<CxxVectorArgument> args = {
...
TEMPLATE_TEST_CASE_METHOD(BfvFixture, "BFV ct_pt_ringt_mac 1", "[.]", BfvTestDefaultParams) {
    for (int m = 2; m <= 20; m++) {
        SECTION("m=" + to_string(m) + "/lv=1") {
            auto cv = new_bfv_test_ct(m, this->ctx, 1, this->param.get_t());
            auto pv = new_bfv_test_pt_ringt(m, this->ctx, this->param.get_t());
            ...
            string path = cpu_base_path + "/" + this->tag + "/BFV_cmpac_1/level_1_m_" + to_string(m);
            FheTaskCpu proj(path);
            vector<CxxVectorArgument> args = {
                {"in_c_list", &cv.ciphertexts},
                {"in_p_list", &pv.plaintexts},
    ...
    SECTION("power_dag " + task_power_str) {
        ...
        for (int level = 5; level <= 5; level++) {
            SECTION("level " + to_string(level)) {
                ...
                string path = cpu_base_path + "/" + this->tag + "/BFV_power_dag/" + task_power_str;
                FheTaskCpu proj(path);
                vector<CxxVectorArgument> args = {
    ...
    SECTION("power_mul_coeff " + task_power_str) {
        ...
        for (int level = 1; level <= 1; level++) {
            SECTION("level " + to_string(level)) {
                ...
                string path = cpu_base_path + "/" + this->tag + "/BFV_power_mul_coeff/" + task_power_str + "/" +
    ...
    SECTION("power_dag_and_power_mul_coeff " + task_power_str) {
        ...
        for (int level = 5; level <= 5; level++) {
            SECTION("level " + to_string(level)) {
                ...
                string path = cpu_base_path + "/" + this->tag + "/BFV_power_dag_and_power_mul_coeff/" + task_power_str +
                              "/" + to_string(lane_cipher_size[0]) + "_" + to_string(lane_cipher_size[1]) + "_" +
                              to_string(lane_cipher_size[2]);
                FheTaskCpu proj(path);
                vector<CxxVectorArgument> args = {
...
TEMPLATE_TEST_CASE_METHOD(BfvFixture, "BFV custom cmpac", "", BfvTestDefaultParams) {
    for (int level = 1; level <= this->max_level; level++) {
        SECTION("lv=" + to_string(level)) {
            ...
            vector<uint64_t> y7_vals = rand_values(n, this->param.get_t());
            ...
            string path = cpu_base_path + "/" + this->tag + "/BFV_custom_cmpac/level_" + to_string(level);
            ...
            cpu_project.run(&this->ctx, cxx_args);

            // z[0][k] = (sum_i(x[i][k] * y[i][k]) + y7[k]) % t
            vector<vector<uint64_t>> expected(1, vector<uint64_t>(n, 0));
            for (int i = 0; i < 7; i++)
                expected[0] = vec_mod_add(expected[0], vec_mod_mul(xv.values[i], y_vals[i], this->param.get_t()),
                                          this->param.get_t());
            expected[0] = vec_mod_add(expected[0], y7_vals, this->param.get_t());
            REQUIRE(decrypt_and_decode(this->ctx, z_list) == expected);
        }
    }
}
