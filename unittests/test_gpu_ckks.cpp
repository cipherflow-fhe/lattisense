...
TEMPLATE_TEST_CASE_METHOD(CkksFixture,
    ...
    for (int level = 1; level <= this->max_level; level++) {
        SECTION("lv=" + to_string(level)) {
            ...
            for (int _i = 0; _i < this->n_op; _i++)
                z_list.push_back(this->ctx.new_ciphertext(level, this->default_scale));
            string path = gpu_base_path + "/" + this->tag + "/CKKS_" + to_string(this->n_op) + "_mult_by_i/level_" +
                          to_string(level);
            FheTaskGpu proj(path);
            vector<CxxVectorArgument> args = {
                {"in_x_list", &xv.ciphertexts},
...
TEMPLATE_TEST_CASE_METHOD(CkksFixture,
    ...
    for (int level = 1; level <= this->max_level; level++) {
        SECTION("lv=" + to_string(level)) {
            ...
            for (int _i = 0; _i < this->n_op; _i++)
                z_list.push_back(this->ctx.new_ciphertext(level, this->default_scale));
            string path = gpu_base_path + "/" + this->tag + "/CKKS_" + to_string(this->n_op) + "_div_by_i/level_" +
                          to_string(level);
            FheTaskGpu proj(path);
            vector<CxxVectorArgument> args = {
                {"in_x_list", &xv.ciphertexts},
...
TEMPLATE_TEST_CASE_METHOD(CkksBtpFixture, "CKKS cmc_relin_rescale_bootstrap", "", CkksBtpParams, CkksSparseBtpParams) {
