...
class TestTask(unittest.TestCase):
    ...
    def test_ct_add_pt_ringt(self, n_op=4, levels=[i for i in range(0, param.max_level + 1)]):
        def cap(x: list[DataNode], y: list[DataNode]) -> DataNode:
            z_list = []
            for i in range(len(x_list)):
                z_list.append(add(x[i], y[i], f'z_{i}'))
    ...
    def test_power_dag(self, levels=[param.max_level]):
        ...
        all_power_dags = []
        power_dag_info = os.listdir(os.path.join(GPU_OUTPUT_BASE_DIR, 'origin_powerdag'))
    ...
    def test_custom_compute_at_start(self, n_op=1, levels=[i for i in range(1, param.max_level + 1)]):
        ...
        for lv in levels:
            with self.subTest(n=n_op, lv=lv):
                task = f'BFV_{n_op}_custom_compute_at_start/level_{lv}'
                task_dir = os.path.join(GPU_OUTPUT_BASE_DIR, task)
    ...
    def test_custom_compute_at_end(self, n_op=4, levels=[i for i in range(1, param.max_level + 1)]):
        def custom_add(x: list[DataNode], y: list[DataNode]) -> DataNode:
            m_list = []
            for i in range(len(x)):
                m_list.append(relin(mult(x[i], y[i], f'x_mult_y_{i}'), f'm_{i}'))

            z_list = []
            for i in range(len(m_list)):
                zi = BfvCiphertextNode(f'z_{i}', level=x[i].level)
                custom_compute(
                    inputs=[m_list[i]],
                    output=zi,
                    type='custom_add',
        ...
        for lv in levels:
            with self.subTest(n=n_op, lv=lv):
                task = f'BFV_{n_op}_custom_compute_at_end/level_{lv}'
                task_dir = os.path.join(GPU_OUTPUT_BASE_DIR, task)
                ...
                z_list = custom_add(x_list, y_list)
    ...
    def test_custom_compute_in_middle(self, n_op=4, levels=[i for i in range(1, param.max_level + 1)]):
        def custom_add(x: list[DataNode], y: list[DataNode]) -> DataNode:
            m_list = []
            for i in range(len(x)):
                m_list.append(relin(mult(x[i], y[i], f'x_mult_y_{i}'), f'm_{i}'))
        ...
        for lv in levels:
            with self.subTest(n=n_op, lv=lv):
                task = f'BFV_{n_op}_custom_compute_in_middle/level_{lv}'
                task_dir = os.path.join(GPU_OUTPUT_BASE_DIR, task)
