...
_CKKS_PARAM_TAGS = {
    ...
    id(_p3): f'ckks_param_default_n{_p3.n}',
    id(_p4): f'ckks_param_default_n{_p4.n}_slots{_p4.slots}',
}

CKKS_PARAMS = [_p1, _p2, _p3, _p4]
...
class TestTask:
    ...
    def test_cap(self, param, lv):
        ...
        task_dir = os.path.join(GPU_OUTPUT_BASE_DIR, param_tag, f'CKKS_{N_OP}_cap', f'level_{lv}')
    ...
    def test_cap_ringt(self, param, lv):
        set_fhe_param(param)
        param_tag = _param_tag(param)
        task_dir = os.path.join(GPU_OUTPUT_BASE_DIR, param_tag, f'CKKS_{N_OP}_cap_ringt', f'level_{lv}')
        x_list = [CkksCiphertextNode(f'x_{i}', level=lv) for i in range(N_OP)]
    ...
    def test_cac(self, param, lv):
        set_fhe_param(param)
        param_tag = _param_tag(param)
        task_dir = os.path.join(GPU_OUTPUT_BASE_DIR, param_tag, f'CKKS_{N_OP}_cac', f'level_{lv}')
        x_list = [CkksCiphertextNode(f'x_{i}', level=lv) for i in range(N_OP)]
    ...
    def test_casc(self, param, lv):
        set_fhe_param(param)
        param_tag = _param_tag(param)
        task_dir = os.path.join(GPU_OUTPUT_BASE_DIR, param_tag, f'CKKS_{N_OP}_casc', f'level_{lv}')
        x_list = [CkksCiphertextNode(f'x_{i}', level=lv) for i in range(N_OP)]
    ...
    def test_csp(self, param, lv):
        set_fhe_param(param)
        param_tag = _param_tag(param)
        task_dir = os.path.join(GPU_OUTPUT_BASE_DIR, param_tag, f'CKKS_{N_OP}_csp', f'level_{lv}')
        x_list = [CkksCiphertextNode(f'x_{i}', level=lv) for i in range(N_OP)]
        y_list = [CkksPlaintextNode(f'y_{i}', level=lv) for i in range(N_OP)]
    ...
    def test_csp_ringt(self, param, lv):
        set_fhe_param(param)
        param_tag = _param_tag(param)
        task_dir = os.path.join(GPU_OUTPUT_BASE_DIR, param_tag, f'CKKS_{N_OP}_csp_ringt', f'level_{lv}')
        x_list = [CkksCiphertextNode(f'x_{i}', level=lv) for i in range(N_OP)]
        y_list = [CkksPlaintextRingtNode(f'y_{i}') for i in range(N_OP)]
        z_list = [sub(x_list[i], y_list[i], f'z_{i}') for i in range(N_OP)]
        process_custom_task(
    ...
    def test_csc(self, param, lv):
        set_fhe_param(param)
        param_tag = _param_tag(param)
        task_dir = os.path.join(GPU_OUTPUT_BASE_DIR, param_tag, f'CKKS_{N_OP}_csc', f'level_{lv}')
        x_list = [CkksCiphertextNode(f'x_{i}', level=lv) for i in range(N_OP)]
        y_list = [CkksCiphertextNode(f'y_{i}', level=lv) for i in range(N_OP)]
        z_list = [sub(x_list[i], y_list[i], f'z_{i}') for i in range(N_OP)]
        process_custom_task(
    ...
    def test_cneg(self, param, lv):
        set_fhe_param(param)
        param_tag = _param_tag(param)
        task_dir = os.path.join(GPU_OUTPUT_BASE_DIR, param_tag, f'CKKS_{N_OP}_cneg', f'level_{lv}')
        x_list = [CkksCiphertextNode(f'x_{i}', level=lv) for i in range(N_OP)]
    ...
    def test_cmp(self, param, lv):
        set_fhe_param(param)
        param_tag = _param_tag(param)
        task_dir = os.path.join(GPU_OUTPUT_BASE_DIR, param_tag, f'CKKS_{N_OP}_cmp', f'level_{lv}')
        x_list = [CkksCiphertextNode(f'x_{i}', level=lv) for i in range(N_OP)]
        y_list = [CkksPlaintextNode(f'y_{i}', level=lv) for i in range(N_OP)]
    ...
    def test_cmp_ringt(self, param, lv):
        set_fhe_param(param)
        param_tag = _param_tag(param)
        task_dir = os.path.join(GPU_OUTPUT_BASE_DIR, param_tag, f'CKKS_{N_OP}_cmp_ringt', f'level_{lv}')
        x_list = [CkksCiphertextNode(f'x_{i}', level=lv) for i in range(N_OP)]
        y_list = [CkksPlaintextRingtNode(f'y_{i}') for i in range(N_OP)]
        z_list = [mult(x_list[i], y_list[i], f'z_{i}') for i in range(N_OP)]
        process_custom_task(
    ...
    def test_ct_pt_mac(self, param, lv):
        ...
        param_tag = _param_tag(param)
        for m in range(2, 21):
            task_dir = os.path.join(GPU_OUTPUT_BASE_DIR, param_tag, f'CKKS_cmpac', f'level_{lv}_m_{m}')
    ...
    def test_ct_pt_ringt_mac(self, param, lv):
        ...
        param_tag = _param_tag(param)
        for m in range(2, 21):
            task_dir = os.path.join(GPU_OUTPUT_BASE_DIR, param_tag, f'CKKS_cmpac_ringt', f'level_{lv}_m_{m}')
            c_list = [CkksCiphertextNode(f'c_{i}', level=lv) for i in range(m)]
    ...
    def test_cmc(self, param, lv):
        set_fhe_param(param)
        param_tag = _param_tag(param)
        task_dir = os.path.join(GPU_OUTPUT_BASE_DIR, param_tag, f'CKKS_{N_OP}_cmc', f'level_{lv}')
        x_list = [CkksCiphertextNode(f'x_{i}', level=lv) for i in range(N_OP)]
        y_list = [CkksCiphertextNode(f'y_{i}', level=lv) for i in range(N_OP)]
        z_list = [mult(x_list[i], y_list[i], f'z_{i}') for i in range(N_OP)]
        process_custom_task(
    ...
    def test_cmc_relin(self, param, lv):
        set_fhe_param(param)
        param_tag = _param_tag(param)
        task_dir = os.path.join(GPU_OUTPUT_BASE_DIR, param_tag, f'CKKS_{N_OP}_cmc_relin', f'level_{lv}')
        x_list = [CkksCiphertextNode(f'x_{i}', level=lv) for i in range(N_OP)]
        y_list = [CkksCiphertextNode(f'y_{i}', level=lv) for i in range(N_OP)]
    ...
    def test_cmc_relin_rescale(self, param, lv):
        set_fhe_param(param)
        param_tag = _param_tag(param)
        task_dir = os.path.join(GPU_OUTPUT_BASE_DIR, param_tag, f'CKKS_{N_OP}_cmc_relin_rescale', f'level_{lv}')
        x_list = [CkksCiphertextNode(f'x_{i}', level=lv) for i in range(N_OP)]
        y_list = [CkksCiphertextNode(f'y_{i}', level=lv) for i in range(N_OP)]
    ...
    def test_csqr(self, param, lv):
        set_fhe_param(param)
        param_tag = _param_tag(param)
        task_dir = os.path.join(GPU_OUTPUT_BASE_DIR, param_tag, f'CKKS_{N_OP}_csqr', f'level_{lv}')
        x_list = [CkksCiphertextNode(f'x_{i}', level=lv) for i in range(N_OP)]
    ...
    def test_csqr_relin(self, param, lv):
        set_fhe_param(param)
        param_tag = _param_tag(param)
        task_dir = os.path.join(GPU_OUTPUT_BASE_DIR, param_tag, f'CKKS_{N_OP}_csqr_relin', f'level_{lv}')
        x_list = [CkksCiphertextNode(f'x_{i}', level=lv) for i in range(N_OP)]
    ...
    def test_csqr_relin_rescale(self, param, lv):
        set_fhe_param(param)
        param_tag = _param_tag(param)
        task_dir = os.path.join(GPU_OUTPUT_BASE_DIR, param_tag, f'CKKS_{N_OP}_csqr_relin_rescale', f'level_{lv}')
        x_list = [CkksCiphertextNode(f'x_{i}', level=lv) for i in range(N_OP)]
    ...
    def test_rescale(self, param, lv):
        set_fhe_param(param)
        param_tag = _param_tag(param)
        task_dir = os.path.join(GPU_OUTPUT_BASE_DIR, param_tag, f'CKKS_{N_OP}_rescale', f'level_{lv}')
        x_list = [CkksCiphertextNode(f'x_{i}', level=lv) for i in range(N_OP)]
    ...
    def test_drop_level(self, param, lv, drop_lv=2):
        ...
        task_dir = os.path.join(
            GPU_OUTPUT_BASE_DIR, param_tag, f'CKKS_{N_OP}_drop_level', f'level_{lv}', f'drop_{drop_lv}'
        )
        x_list = [CkksCiphertextNode(f'x_{i}', level=lv) for i in range(N_OP)]
    ...
    def test_rotate_col(self, param, lv, steps=[i + 1 for i in range(8)]):
        ...
        param_tag = _param_tag(param)
        task_dir = os.path.join(
            GPU_OUTPUT_BASE_DIR, param_tag, f'CKKS_{N_OP}_rotate_col', f'level_{lv}', f'steps_{steps[0]}_to_{steps[-1]}'
        )
        x_list = [CkksCiphertextNode(f'x_{i}', level=lv) for i in range(N_OP)]
    ...
    def test_advanced_rotate_col(self, param, lv, steps=[-500, 20, 200, 2000, 4000]):
        ...
        task_dir = os.path.join(
            GPU_OUTPUT_BASE_DIR, param_tag, f'CKKS_{N_OP}_advanced_rotate_col', f'level_{lv}', f'steps_{steps_str}'
        )
        x_list = [CkksCiphertextNode(f'x_{i}', level=lv) for i in range(N_OP)]
    ...
    def test_rotate_row(self, param, lv):
        set_fhe_param(param)
        param_tag = _param_tag(param)
        task_dir = os.path.join(GPU_OUTPUT_BASE_DIR, param_tag, f'CKKS_{N_OP}_rotate_row', f'level_{lv}')
        x_list = [CkksCiphertextNode(f'x_{i}', level=lv) for i in range(N_OP)]
    ...
    def test_cap_2d(self, param, lv, row=1, col=1):
        ...
        set_fhe_param(param)
        param_tag = _param_tag(param)
        task_dir = os.path.join(GPU_OUTPUT_BASE_DIR, param_tag, f'CKKS_cap_row_{row}_col_{col}', f'level_{lv}')
    ...
    def test_cap_3d(self, param, lv, dim0=1, dim1=1, dim2=1):
        ...
        set_fhe_param(param)
        param_tag = _param_tag(param)
        task_dir = os.path.join(GPU_OUTPUT_BASE_DIR, param_tag, 'CKKS_cap_3d', f'level_{lv}')
    ...
    def test_cmc_relin_4d(self, param, lv, dim0=1, dim1=1, dim2=1, dim3=1):
        ...
        set_fhe_param(param)
        param_tag = _param_tag(param)
        task_dir = os.path.join(GPU_OUTPUT_BASE_DIR, param_tag, 'CKKS_cmc_relin_4d', f'level_{lv}')
        x_ct_4d = [
    ...
    def test_cmp_3d(self, param, lv, dim0=1, dim1=1, dim2=1):
        ...
        set_fhe_param(param)
        param_tag = _param_tag(param)
        task_dir = os.path.join(GPU_OUTPUT_BASE_DIR, param_tag, 'CKKS_cmp_3d', f'level_{lv}')
        x_ct_3d = [[[CkksCiphertextNode(level=lv) for _ in range(dim2)] for _ in range(dim1)] for _ in range(dim0)]
        y_pt_3d = [[[CkksPlaintextNode(level=lv) for _ in range(dim2)] for _ in range(dim1)] for _ in range(dim0)]
        z_ct_3d = [
    ...
    def test_cmp_4d(self, param, lv, dim0=1, dim1=1, dim2=1, dim3=1):
        ...
        set_fhe_param(param)
        param_tag = _param_tag(param)
        task_dir = os.path.join(GPU_OUTPUT_BASE_DIR, param_tag, 'CKKS_cmp_4d', f'level_{lv}')
        x_ct_4d = [
            [[[CkksCiphertextNode(level=lv) for _ in range(dim3)] for _ in range(dim2)] for _ in range(dim1)]
            for _ in range(dim0)
        ]
...
class TestBootstrap:
    ...
    def test_toy_bootstrap(self, lv):
        ...
        task_dir = os.path.join(GPU_OUTPUT_BASE_DIR, param_tag, f'CKKS_{N_OP}_toy_bootstrap', f'level_{lv}')
        x_list = [CkksCiphertextNode(f'x_{i}', level=lv) for i in range(N_OP)]
    ...
    def test_bootstrap(self, lv):
        ...
        task_dir = os.path.join(GPU_OUTPUT_BASE_DIR, param_tag, f'CKKS_{N_OP}_bootstrap', f'level_{lv}')
        x_list = [CkksCiphertextNode(f'x_{i}', level=lv) for i in range(N_OP)]
        y_list = [bootstrap(x_list[i], f'y_{i}') for i in range(N_OP)]
        process_custom_task(
            input_args=[Argument('in_x_list', x_list)],
    ...
    def test_cmc_relin_rescale_bootstrap(self, lv):
        ...
        param_tag = f'ckks_param_btp_n{_p_btp.n}'
        task_dir = os.path.join(
            GPU_OUTPUT_BASE_DIR, param_tag, f'CKKS_{N_OP}_cmc_relin_rescale_bootstrap', f'level_{lv}'
        )
        x_list = [CkksCiphertextNode(f'x_{i}', level=lv) for i in range(N_OP)]
        y_list = [CkksCiphertextNode(f'y_{i}', level=lv) for i in range(N_OP)]
        result_list = []
        for i in range(N_OP):
            ...
            z_dropped = drop_level(z_rescaled, 2)
