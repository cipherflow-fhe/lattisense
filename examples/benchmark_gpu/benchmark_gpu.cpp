...
void benchmark_bfv_mult_relin() {
    ...
    FheTaskGpu task("bfv_mult_relin");
    std::vector<CxxVectorArgument> args = {{"xs", &xs}, {"ys", &ys}, {"zs", &zs}};
    uint64_t time_us = task.run(&ctx, args, false);

    printf("BFV mult_relin: %d ops, %.2f ms, %.1f ops/sec\n", n_op, time_us / 1000.0, n_op / (time_us / 1e6));
}
...
void benchmark_ckks_mult_relin() {
    ...
    FheTaskGpu task("ckks_mult_relin");
    std::vector<CxxVectorArgument> args = {{"xs", &xs}, {"ys", &ys}, {"zs", &zs}};
    uint64_t time_us = task.run(&ctx, args, false);
...
void benchmark_bfv_rotate_col() {
    ...
    FheTaskGpu task("bfv_rotate_col");
...
int main(int argc, char* argv[]) {
    const char* help = "Usage: benchmark_gpu <0|1|2|all>\n"
                       "  0: BFV mult_relin\n"
                       "  1: CKKS mult_relin\n"
                       "  2: BFV rotate_col\n"
