...
enum class OperationType {
    ...
    MAC_W_PARTIAL_SUM,
    BOOTSTRAP,
    POLY_EVAL,  // <--- 新增：多项式求值算子，通用，只要用多项式近似就可以用这个
    NEWTON_RECIPROCAL,    // <--- 新增：迭代求逆算子
...
struct ComputeNode {
    ...
    struct ExtraProperty {
        ...
        // 新增用于多项式近似的字段
        int poly_degree = 8;
        double poly_left = -5.0;
        double poly_right = 5.0;
        // std::string poly_func;
        std::string poly_func = "exp";
        std::vector<double> poly_coeffs;   // 新增：切比雪夫系数列表

        // 新增用于迭代法一致性，给出默认值
        int newton_iterations = 4;
        double newton_init_guess = 1.0;  // 单值初值

        // std::vector<double> newton_init_guess = {0.8}; // 向量化，但是仍是一个单值向量

        int goldschmidt_iterations = 4;
    };
    std::optional<ExtraProperty> p;
};
