...
const std::unordered_map<std::string, OperationType> str_to_operation_type = {
    ...
    //{"exp", OperationType::POLY_EXP}, 
    {"poly_eval", OperationType::POLY_EVAL},  // 新增：将 Python 端的 "poly_eval" 映射到 POLY_EVAL
    {"newton_reciprocal", OperationType::NEWTON_RECIPROCAL}, // 新增：将 Python 端的 "newton_reciprocal" 映射到 NEWTON_RECIPROCAL
...
MegaAG MegaAG::from_json(const std::string& json_path, Processor processor) {
    ...
    for (auto& [key, value] : computes_json.items()) {
        ...
        // 完成参数传递，构建一个指针，使前面进行调用的时候能够正确获取这些值并进一步传递给真正计算的部分
        // else if (node.op_type == OperationType::POLY_EXP) {
        //     ComputeNode::ExtraProperty prop;
        //     // 使用 nlohmann/json 的 value 方法，提供默认值防止 key 不存在时崩溃
        //     prop.poly_degree = value.value("degree", 8);
        //     prop.poly_left = value.value("left", -5.0);
        //     prop.poly_right = value.value("right", 5.0);
        //     prop.poly_func = value.value("func", "exp");
        //     node.p = prop; // 将填充好的属性赋值给节点
        // }
        else if (node.op_type == OperationType::POLY_EVAL) {
            ComputeNode::ExtraProperty prop;
            // 提供默认值防止 key 不存在时崩溃，仅在无传入时使用
            prop.poly_degree = value.value("degree", 8);
            prop.poly_left   = value.value("left", -5.0);
            prop.poly_right  = value.value("right", 5.0);
            prop.poly_func   = value.value("func", "exp");
            ...
            // 新增：读取系数数组
            if (value.contains("poly_coeffs") && value["poly_coeffs"].is_array()) {
                prop.poly_coeffs = value["poly_coeffs"].get<std::vector<double>>();
            }
        ...
        // 迭代法读取参数
        else if (node.op_type == OperationType::NEWTON_RECIPROCAL) {
            ComputeNode::ExtraProperty prop;
            prop.newton_iterations = value.value("iterations", 4);
            prop.newton_init_guess = value.value("init_guess", 1.0);
            node.p = prop;
        }

        // 迭代法读取参数
        else if (node.op_type == OperationType::GOLDSCHMIDT_RECIPROCAL) {
            ComputeNode::ExtraProperty prop;
            prop.goldschmidt_iterations = value.value("iterations", 4);
            node.p = prop;
        }
